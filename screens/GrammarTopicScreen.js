...
const RuleCard = ({ rule, levelColor }) => (
  <View style={[styles.ruleCard, { borderLeftColor: levelColor }]}>
    <Text style={styles.ruleTitle}>{rule.rule}</Text>
    <Text style={styles.ruleExplanation}>{rule.explanation}</Text>
    {rule.example && (
      <View style={[styles.exampleBox, { backgroundColor: `${levelColor}15` }]}>
        <Text style={[styles.exampleText, { color: levelColor }]}>{rule.example}</Text>
      </View>
    )}
  </View>
);

const ExampleCard = ({ example, index, levelColor }) => (
  <View style={styles.exampleCard}>
    <View style={[styles.exampleNumber, { backgroundColor: levelColor }]}>
...
const GrammarTopicScreen = ({ route, navigation }) => {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {/* ✅ ВИПРАВЛЕННЯ #10: колір активного табу = level.color, не хардкоджений зелений */}
      <View style={styles.tabBar}>
        {[
          { key: 'theory', label: '📚 Теорія' },
          { key: 'examples', label: '💡 Приклади' },
          ...
          <TouchableOpacity
            ...
            style={[
              styles.tab,
              activeTab === tab.key && { borderBottomColor: level.color },
            ]}
            onPress={() => setActiveTab(tab.key)}
          >
            <Text style={[
              styles.tabText,
              activeTab === tab.key && { color: level.color },
            ]}>
              {tab.label}
            </Text>
          </TouchableOpacity>
            ...
            {topic.explanation?.rules?.length > 0 && (
              <View style={styles.section}>
                ...
                {topic.explanation.rules.map((rule, idx) => (
                  <RuleCard key={idx} rule={rule} levelColor={level.color} />
                ))}
              </View>
            )}

            {topic.explanation?.tables?.length > 0 && (
              <View style={styles.section}>
          ...
          <View style={styles.tabContent}>
            {topic.examples?.length > 0 ? (
              topic.examples.map((example, idx) => (
                <ExampleCard key={idx} example={example} index={idx} levelColor={level.color} />
              ))
            ) : (
...
const styles = StyleSheet.create({
  ...
  levelBadge: { backgroundColor: 'rgba(255,255,255,0.3)', paddingHorizontal: 12, paddingVertical: 4, borderRadius: 12, color: '#fff', fontSize: 12, fontWeight: '700', marginBottom: 8 },
  headerTitle: { fontSize: 24, fontWeight: 'bold', color: '#fff', textAlign: 'center' },
  headerSubtitle: { fontSize: 16, color: '#fff', opacity: 0.9, marginTop: 4, textAlign: 'center' },
  ...
  introText: { fontSize: 15, lineHeight: 22, color: '#555', backgroundColor: '#fff', padding: 15, borderRadius: 12 },
  ruleCard: { backgroundColor: '#fff', padding: 15, borderRadius: 12, marginBottom: 10, borderLeftWidth: 4 },
  ruleTitle: { fontSize: 16, fontWeight: 'bold', color: '#333', marginBottom: 6 },
  ruleExplanation: { fontSize: 14, color: '#666', marginBottom: 8 },
  exampleBox: { padding: 10, borderRadius: 8, marginTop: 8 },
  exampleText: { fontSize: 14, fontStyle: 'italic' },
  tableContainer: { backgroundColor: '#fff', borderRadius: 12, overflow: 'hidden', marginBottom: 15 },
  tableTitle: { fontSize: 15, fontWeight: '600', color: '#333', padding: 12, backgroundColor: '#f5f5f5' },
  ...
  tableHeaderText: { fontSize: 13, fontWeight: '700', color: '#555' },
  tableCellText: { fontSize: 13, color: '#666' },
  tipItem: { flexDirection: 'row', marginBottom: 8, paddingLeft: 5 },
  tipBullet: { fontSize: 16, marginRight: 8, marginTop: 2 },
  tipText: { flex: 1, fontSize: 14, lineHeight: 20, color: '#555' },
  mistakeCard: { backgroundColor: '#fff', padding: 15, borderRadius: 12, marginBottom: 10 },
  mistakeWrong: { fontSize: 14, color: '#d32f2f', marginBottom: 4 },
  mistakeCorrect: { fontSize: 14, color: '#2e7d32', marginBottom: 6 },
  mistakeExplanation: { fontSize: 13, color: '#666', fontStyle: 'italic' },
  exampleCard: { flexDirection: 'row', backgroundColor: '#fff', padding: 15, borderRadius: 12, marginBottom: 10 },
  exampleNumber: { width: 30, height: 30, borderRadius: 15, justifyContent: 'center', alignItems: 'center', marginRight: 12 },
  exampleNumberText: { color: '#fff', fontSize: 14, fontWeight: 'bold' },
  exampleContent: { flex: 1 },
  exampleEnglish: { fontSize: 15, fontWeight: '600', color: '#333', marginBottom: 4 },
  exampleUkrainian: { fontSize: 14, color: '#666' },
  exercisesInfo: { fontSize: 16, color: '#555', textAlign: 'center', marginBottom: 15 },
  startButton: { padding: 16, borderRadius: 12, alignItems: 'center', marginBottom: 20 },
  startButtonText: { color: '#fff', fontSize: 18, fontWeight: 'bold' },
  exercisesList: { marginTop: 10 },
  exerciseItem: { flexDirection: 'row', backgroundColor: '#fff', padding: 15, borderRadius: 12, marginBottom: 10 },
  exerciseIcon: { fontSize: 24, marginRight: 12 },
  exerciseInfo: { flex: 1 },
  exerciseTitle: { fontSize: 15, fontWeight: '600', color: '#333', marginBottom: 4 },
  exerciseCount: { fontSize: 13, color: '#888' },
